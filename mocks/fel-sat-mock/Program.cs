...
app.MapPost("/postAnulacionDTE", async (HttpRequest request) =>
{
    ...
    return Results.Ok(new
    {
        ...
        uuid = id,
        fecha = record.ReceivedAt,
        mensaje = "Anulacion recibida correctamente por mock"
    });
});
...
{
    new { code = "TC", name = "Tarjeta de Credito" },
    new { code = "TD", name = "Tarjeta de Debito" },
...
static bool TryValidateAccessToken(HttpRequest request, ConcurrentDictionary<string, DateTimeOffset> tokens)
{
    ...
    return tokens.TryGetValue(token, out var expiresAt) && expiresAt > DateTimeOffset.UtcNow;
}
